...
11. 每个 catalog family 都有 server/client provider gate，公共入口不链接 optional 实现。
12. 输入事务、smart-pattern 容量和输出回网继续走共同 support/router，不回退到具体机器分支。
13. `runData` 连续执行结果稳定，特殊手写模型不被通用生成资源覆盖。
