...
后续适配 Mekanism More Machine、Mekanism Extras 或其它机器模组时，统一按 `CompatMachineFamily -> CompatMachineCatalog -> provider/family adapter -> BlockEntity I/O adapter -> Data Generation` 接入。公共注册、菜单、客户端和安装器不再为每台机器增加分散判断。
...
| [06 - 样板输入与 I/O adapter 规则](06-pushPattern编写规则.md) | 声明式输入布局、事务路由 |
| [07 - 输出回网规则](07-输出回网规则.md) | AeOutputMode、powered insert、ticker 唤醒 |
| [08 - Mixin accessor](08-MixinAccessor.md) | 暴露 Mek 私有字段 |
| [09 - BlockType、Side Config 和默认物品配置](09-BlockType和SideConfig.md) | catalog profile、provider builder |
| [10 - 菜单和客户端 GUI](10-菜单和客户端GUI.md) | provider、client provider、样板按钮 |
...
| [20 - 新增其它模组 compat 的模板](20-新增其它模组compat模板.md) | provider、family adapter、接入位置 |
| [21 - 可选模组探测规则](21-可选模组探测规则.md) | 集中 runtime probe + class resource |
...
| [28 - 工厂 I/O adapter 与共同支撑层](28-外部工厂共用支撑层.md) | MeFactoryIoOwner / MeFactoryAeSupport |
...
| [33 - 兼容机器目录](33-兼容机器目录.md) | 集中机器元数据、注册路由与可选依赖边界 |
