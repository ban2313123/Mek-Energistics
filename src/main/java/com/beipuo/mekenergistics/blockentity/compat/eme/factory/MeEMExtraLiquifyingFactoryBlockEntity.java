...
public class MeEMExtraLiquifyingFactoryBlockEntity extends TileEntityEMExtraLiquifyingFactory implements MeEvolvedMekanismExtrasFactoryAeMachine {
    private final MeMekanismMachine machine;
    private MeFactoryAeSupport aeSupport;

    public MeEMExtraLiquifyingFactoryBlockEntity(MeMekanismMachine machine, BlockPos pos, BlockState state) {
    ...
    @Override public boolean pushPattern(IPatternDetails pattern, KeyCounter[] input) { return getMainNode().isActive() && getAvailablePatterns().contains(pattern) && (isSmartPatternMultiplicationEnabled() ? getAeSupport().enqueueSmartPattern(pattern, input) : getAeSupport().pushSingleItem(input, this.inputItemSlots)); }
    ...
    @Override public CachedRecipe<BasicItemStackToFluidOptionalItemRecipe> createNewCachedRecipe(BasicItemStackToFluidOptionalItemRecipe recipe, int cacheIndex) { return MeFactoryAeSupport.withAeRecipeEnergy(this, this.energyContainer, super.createNewCachedRecipe(recipe, cacheIndex)); }
    @Override protected boolean onUpdateServer() { boolean changed = super.onUpdateServer(); return getAeSupport().processSingleItemSmartPatterns(this.outputItemSlots, this.fluidTank, this.inputItemSlots) || changed; }
