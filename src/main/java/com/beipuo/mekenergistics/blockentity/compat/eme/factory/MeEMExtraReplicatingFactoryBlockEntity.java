...
/** AE adapter for EMEKE MekMM replicating factories. */
public class MeEMExtraReplicatingFactoryBlockEntity extends TileEntityEMExtraReplicatingFactory implements MeEvolvedMekanismExtrasFactoryAeMachine {
    ...
    @Override public boolean pushPattern(IPatternDetails p, KeyCounter[] in) { return getMainNode().isActive() && getAvailablePatterns().contains(p) && (isSmartPatternMultiplicationEnabled() ? getAeSupport().enqueueSmartPattern(p, in) : getAeSupport().pushItemChemicalOrConversion(in, this.inputSlots, this.chemicalTank, getExtraSlot())); }
