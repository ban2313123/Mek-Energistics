...
当前公共注册入口由 `CompatMachineCatalog` 驱动。目录只保存纯数据；具体第三方类必须留在分族
provider 中。菜单和客户端 screen 通过 feature gate 后再加载 optional provider，不能在
`ModMenuTypes` 或 `ClientSetup` 的公共静态字段中出现可选 BlockEntity 的 class literal。
BlockEntity、BlockType、Grid host 与 installer 的公共入口只能依赖 `CompatMachineProvider`；
feature gate 缺失时，对应机器不会进入 `available()`，也不会加载其 provider 路径。
