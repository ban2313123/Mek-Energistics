...
5. 通过 `MeFactoryAeSupport` 声明主输入槽列表；support 会将 Mekanism 自动均分槽组合成共同事务端口，extra/catalyst 槽继续声明为独立 lane。
...
8. BlockEntity 继承目标模组真实 tile，例如 `TileEntityExtraAlloyingFactory`，实现 `MeFactoryIoOwner`，再通过 `MeFactoryAeSupport` 声明输入 lane、能量包装和实际输出端口。
